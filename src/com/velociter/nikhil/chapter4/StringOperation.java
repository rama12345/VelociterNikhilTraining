...
public class StringOperation {
	public static void main(String args[]) {
		...
		for (i = 0; i < lenghtOfString; i++) {
			characterOfString = textOfString.charAt(i); // breaks the string into the characters and iterated on the
														// each characters one by one
		...
		for (i = 0; i < numberOfWords; i++) {
			...
			if (largeWords.length() < arrayOfString[i].length())// compare the length of the string and assign the
																// string into the variable samllWords and largeWords
